...
import { Heart, BookOpen, Trash2, Share2, Play } from "lucide-react";
...
function StoriesPage() {
  ...
  const toggle = (id: string) => { setStories(store.toggleFav(id)); toast.success("Favorites updated"); };
  const remove = (id: string) => { setStories(store.removeStory(id)); toast("Story removed"); };
  const share = (s: Story) => {
        ...
        {stories.map((s, i) => (
          <motion.article key={s.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} whileHover={{ y: -6 }}
            ...
            <div className={`relative grid h-44 place-items-center bg-gradient-to-br ${s.cover} text-white`}>
              ...
              <button onClick={() => toggle(s.id)} aria-label="Favorite" className="absolute left-3 top-3 grid size-9 place-items-center rounded-2xl bg-white/30 backdrop-blur transition hover:scale-110">
            ...
            <div className="p-4">
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  ...
                  <button onClick={() => remove(s.id)} aria-label="Delete" className="grid size-8 place-items-center rounded-xl text-muted-foreground hover:bg-magic-pink/10 hover:text-magic-pink"><Trash2 className="size-4" /></button>
                </div>
              </div>
            </div>
          </motion.article>
